"""
Refresh Freedom Power processed dataset.

Pulls paid media + leads from BigQuery, GQV from GCS, merges static controls,
applies billboard corrections, and writes a model-ready CSV.

Test mode (default):
  BQ pull limited to last 16 weeks for fast validation.
  Output: data/processed/Freedom_Power/test_refresh.csv

Production mode (--prod):
  Full pull from 2024-01-01 to current date.
  Output: data/processed/Freedom_Power/Freedom_MMM_data_{vintage}_gqv.csv

Usage:
    source .venv/bin/activate
    python scripts/refresh_freedom_power.py           # test
    python scripts/refresh_freedom_power.py --prod    # production
"""
...
def validate(df: pd.DataFrame) -> bool:
    checks = [
        ...
        ((df["Gross_Leads"] >= 0).all(),                        "Gross_Leads non-negative"),
    ]
    all_passed = True
...
def main():
    ...
    if args.prod:
        ...
        out_path = out_dir / f"Freedom_MMM_data_{vintage}_gqv.csv"
    else:
        out_path = out_dir / "test_refresh.csv"
    ...
    if args.prod:
        ...
        print(f"\nNext: update notebook cell 1 path to:")
        print(f'  pd.read_csv("data/processed/Freedom_Power/Freedom_MMM_data_{vintage_str}_gqv.csv")')
