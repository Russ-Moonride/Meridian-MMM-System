"""
Refresh Freedom Power Sets dataset (KPI = HubSpot_Sets).

Identical pipeline to refresh_freedom_power.py but pulls HubSpot_Sets
instead of Gross_Leads. Used to fit the downstream Sets MMM model.

Test mode (default):
  BQ pull limited to last 16 weeks for fast validation.
  Output: data/processed/Freedom_Power/test_refresh_sets.csv

Production mode (--prod):
  Full pull from 2024-01-01 to current date.
  Output: data/processed/Freedom_Power/Freedom_MMM_sets_{vintage}_gqv.csv

Usage:
    source .venv/bin/activate
    python scripts/refresh_freedom_power_sets.py           # test
    python scripts/refresh_freedom_power_sets.py --prod    # production
"""
...
def validate(df: pd.DataFrame) -> bool:
    checks = [
        ...
        ((df["HubSpot_Sets"] >= 0).all(),                        "HubSpot_Sets non-negative"),
    ]
    all_passed = True
...
def main():
    ...
    if args.prod:
        ...
        out_path = out_dir / f"Freedom_MMM_sets_{vintage}_gqv.csv"
    else:
        out_path = out_dir / "test_refresh_sets.csv"
    ...
    if args.prod:
        ...
        print(f"\nNext: update configs/Freedom_Power_Sets.yaml data_path to:")
        print(f'  data/processed/Freedom_Power/Freedom_MMM_sets_{vintage_str}_gqv.csv')
